...
export async function readWorkBook<T>({
    fileName,
    sheetName,
    clazz
}: {
    ...
    let workbook = await openWorkbook(fileName);
    let sheet = workbook.getWorksheet(sheetName);
    return await importRecords(sheet, () => new clazz());
}

export async function writeWorkBook<T>({
    fileName,
    sheetName,
    records
}: {
    ...
    await exportRecords(sheet, records);
