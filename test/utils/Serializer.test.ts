import { Serializer, Serializable, transiant } from '../../src/utils/Serializer';
...
//   console.log(b1);
  ...
  class C{
      ...
      serialize():string{
          return JSON.stringify({text: this.value})
      }
      deserialize(text:string){
          this.value = JSON.parse(text).text
      }
  }
  let cstr = Serializer.serialize(new C('ddafafas'));
  console.log(cstr);
  console.log(Serializer.deserialize(cstr));
}
test();
